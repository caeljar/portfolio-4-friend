...
const Projects: React.FC = () => {
    ...
    return (
        <section id="projects" className="w-full min-h-screen bg-secondary text-primary py-24 flex flex-col justify-center">
            <div className="container mx-auto px-6 md:px-12 flex flex-col md:flex-row gap-12 min-h-[75vh] md:h-[75vh]">
                ...
                <div className="w-full md:w-1/3 flex flex-row md:flex-col overflow-x-auto md:overflow-visible no-scrollbar pb-4 md:pb-0 space-x-6 md:space-x-0 md:space-y-8 border-b md:border-b-0 md:border-l border-primary/20 md:pl-8 relative snap-x snap-mandatory">
                    <div className="hidden md:block absolute left-[-1px] top-0 h-full w-[2px] bg-transparent">
                        {/* Animated Indicator */}
                        <motion.div
                            className="w-full bg-primary absolute"
                            initial={false}
                            animate={{
                                top: `${(activeIndex / portfolioData.projects.length) * 100}%`,
                                height: `${100 / portfolioData.projects.length}%`
                            }}
                    ...
                    {portfolioData.projects.map((project, index) => (
                        <button
                            ...
                            className={`text-left text-2xl whitespace-nowrap snap-start md:whitespace-normal md:text-5xl font-bold tracking-tighter uppercase transition-colors duration-500 ${activeIndex === index ? 'text-primary' : 'text-primary/30 hover:text-primary/60'
                                }`}
                        >
